...
// --- Componente Toast para una notificación simple y elegante ---
const Toast = ({ message, onHide }) => {
  useEffect(() => {
    const timer = setTimeout(onHide, 3000); // Ocultar después de 3 segundos
    return () => clearTimeout(timer);
  }, [onHide]);

  return (
    <div className="fixed top-24 right-8 bg-white rounded-lg shadow-xl p-4 flex items-center space-x-3 z-50 animate-fade-in-right">
...
const SeatSelector = ({ funcion, salaInfo, boletosOcupados, usuario }) => {
  const [selectedSeats, setSelectedSeats] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');
  const [showSuccessToast, setShowSuccessToast] = useState(false); // Estado para el Toast
  
  const PRECIO_POR_BOLETO = 15;
  ...
  const handlePurchase = async () => {
    ...
    const boletosToInsert = selectedSeats.map(seat => ({
    ...
    const { error: insertError } = await supabase.from('boletos').insert(boletosToInsert);

    if (insertError) {
      console.error('Error al guardar:', insertError);
      setError(`Error al procesar tu compra. Por favor, intenta nuevamente.`);
      setIsLoading(false);
    } else {
      setShowSuccessToast(true); // Mostramos el Toast de éxito
      setTimeout(() => {
        window.location.href = '/perfil';
      }, 3000); // Redirigimos después de que el Toast se oculte
    }
  };
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    <span>{usuario ? 'Confirmar Compra' : 'Iniciar Sesión para Continuar'}</span>
                  </>
                )}
              </button>
            </div>
