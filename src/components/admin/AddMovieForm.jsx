  ...
  const handleSubmit = async (e) => {
    ...
    // ENDPOINT POST: Insertar nueva película
      const { error: insertError } = await supabase
        .from('peliculas')
        .insert([
          { ...formData, duracion: parseInt(formData.duracion) }
        ]);
      ...
      setSuccess('¡Película añadida exitosamente!');
      setTimeout(() => {
        window.location.href = '/admin/peliculas';
      }, 1500);
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-4">
          ...
          >
            ...
            {isLoading ? 'Guardando...' : 'Añadir Película'}
          </button>
        </div>
      </form>
    </div>
